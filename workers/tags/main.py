...
from tubedigest.contracts import WorkerMessage, QUEUE_VIDEO_TAGS
from workers.tags.handler import handle_message, HandlerDeps, Database, Publisher
from workers.tags.adapters import Tagger, FakeTagger
...
class SqliteJobStore:
    ...
    def get_job(self, job_id: str) -> Optional[Dict[str, Any]]:
        with self._conn() as conn:
            row = conn.execute(
                "SELECT id, transcript, summary, tags, chapters "
...
def _process_message(ch, method, body, deps: HandlerDeps) -> None:
    try:
        ...
        logging.exception("tags: failed to parse message")
    ...
    try:
        ...
        logging.exception("tags: failed to handle message")
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
        return
...
def main() -> None:
    ...
    channel.queue_declare(queue=QUEUE_VIDEO_TAGS, durable=True)
